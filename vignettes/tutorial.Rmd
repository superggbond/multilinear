...
Using another function `coefs` in our package, we can also provide a brief comparable estimates of all coefficients.     
The estimates of coefficients using `coefs`:
```{r, echo = TRUE}
coefs(fit.mlr)
```

The estimates of coefficients using `lm`:
```{r, echo = TRUE}
coefficients(fit.lm)
```
